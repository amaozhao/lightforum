...
class TopicSerializer(SimpleTopicSerializer):
    ...
    author_followings = serializers.SerializerMethodField(
        'get_author_followings')
    ...
    addcommentavatar = serializers.SerializerMethodField(
        'get_addcommentavatar')
    ...
    def get_addcommentavatar(self, obj):
        try:
            return self.context[
                'view'].request.user.get_profile().get_avatar(size=48)
        except:
    ...
    def get_cut_markdown(self, obj):
        markdown = Truncator(obj.get_content()).words(
            40, html=True, truncate=' ...')
        if markdown == obj.get_content():
            return markdown
        return Truncator(obj.get_content()).words(
            40, html=True, truncate=' ...')

    class Meta:
        model = Topic
        fields = ('id', 'author', 'author_id', 'author_fans',
                  'author_followings',
